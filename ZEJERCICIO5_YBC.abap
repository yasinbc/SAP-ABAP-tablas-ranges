...
data: clientes type table of ztybc_clientes,
      cliente like line of clientes,
      "compia exactamente el mismo tipo del campo SALDO en la tabla interna ZTYBC_CLIENTES
      "heredando sus caracteristicas a la nueva variable TOTAL
...
ids-low = '1'. "llamamos la propiedad 'low' (valor "desde" del intervalo) (Operador de comparacion) de RANGES y le asignamos la opcion '1' (por donde va a empezar)
ids-high = '3'. "llamamos la propiedad 'high' (valor "hasta" del intervalo, solo si se usa BETWEEN, si no, no se usa) de RANGES y le asignamos la opcion '3' (hasta donde)

"Introduciomos la configuracion de rangos 'ids' al final de la tabla
append ids.

"hacemos la seleccion dentro de la tabla utilizando RANGES preedterminado
select clavecliente from ztybc_clientes where clavecliente in @ids"@ids apunta a la configuracion ya hecha del rango
...
select sum( saldo ) from @clientes as cliente into @total.
